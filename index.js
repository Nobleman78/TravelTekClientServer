...
async function run() {
  try {
    ...
    app.post('/users', async (req, res) => {
      ...
      user.role = (user.email === 'jesminchakma@gmail.com') ? 'admin' : 'user';
      ...
      try {
        ...
        if (existingClient) {
          if (existingClient.purpose.trim().toLowerCase() !== purpose.trim().toLowerCase()) {
            const updateResult = await clientCollection.updateOne(
              { phoneNumber },
              { $set: { purpose, updatedAt: new Date().toISOString() } }
            );
            return res.send({
              message: 'Purpose updated for existing client.',
              updated: true,
        ...
        const result = await clientCollection.insertOne(timestampedClient);
        res.send({ message: 'New client added successfully.', inserted: true, result });
...
// Root route - simple health check or welcome
app.get('/', (req, res) => {
  res.send('Server Is Running');
});

// SPA support: serve frontend index.html on all non-API routes
app.get('*', (req, res) => {
  // Ignore API routes
  if (req.path.startsWith('/jwt') || req.path.startsWith('/users') || req.path.startsWith('/client-information')) {
